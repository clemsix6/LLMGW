...
// key create has not yet made. Setting a project setting never creates one:
...
func (s *Store) Projects(ctx context.Context) ([]governance.Project, error) {
	const query = `
SELECT id, name, created_at, COALESCE(default_effort, ''), reject_tool_markup
FROM project
ORDER BY name`
...
func scanProject(row pgx.Row) (governance.Project, error) {
	...
	err := row.Scan(
		&project.ID, &project.Name, &project.CreatedAt, &project.DefaultEffort,
